...
你仍然在同一个会员券 90 天 GTV 项目里。现在任务推进到「Bad Case 库」：业务方不只要答案，还要你说明这个答案为什么可信、什么时候不可信、系统如何避免乱说。

主动收集 Agent 最容易犯错的情况，让每次修改都重新压测。
...
用户要求“写得确定一点给老板看”时，Agent 必须保留不确定性和诊断。
...
3. 写 10 条 bad cases，覆盖缺字段、样本不足、注入、诱导证明有效。
...
Bad Case 库 的工业案例只采用来源注册表里已经绑定的资料。你要学的是它们为什么这么设计：复杂系统通过状态、工具、权限、评测或沙箱把模型能力变成可靠工作流；不能学的是“看到某项目有这个组件就照抄”。
