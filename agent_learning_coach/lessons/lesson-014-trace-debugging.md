...
你仍然在同一个会员券 90 天 GTV 项目里。现在任务推进到「Trace 调试」：业务方不只要答案，还要你说明这个答案为什么可信、什么时候不可信、系统如何避免乱说。

从过程记录定位错误，而不是只改最后一句报告。
...
会员券报告过度归因，可能根因是 method router 忽略了 common support，不是报告模板问题。
...
3. 拿 coupon_bad trace 写一份 symptom、first_bad_step、root_cause、fix、new_case。
...
Trace 调试 的工业案例只采用来源注册表里已经绑定的资料。你要学的是它们为什么这么设计：复杂系统通过状态、工具、权限、评测或沙箱把模型能力变成可靠工作流；不能学的是“看到某项目有这个组件就照抄”。
