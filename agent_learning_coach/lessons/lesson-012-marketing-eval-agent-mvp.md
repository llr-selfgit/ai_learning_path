...
你仍然在同一个会员券 90 天 GTV 项目里。现在任务推进到「营销评估 Agent MVP」：业务方不只要答案，还要你说明这个答案为什么可信、什么时候不可信、系统如何避免乱说。

跑通 inspect、select method、estimate、diagnose、report 的端到端链路。
...
会员券 good 数据可以进入估计；bad 数据必须降级。报告同时展示结论、诊断、限制和下一步建议。
...
3. 准备一份 MVP trace 样例，包含每个节点的输入输出。
...
营销评估 Agent MVP 的工业案例只采用来源注册表里已经绑定的资料。你要学的是它们为什么这么设计：复杂系统通过状态、工具、权限、评测或沙箱把模型能力变成可靠工作流；不能学的是“看到某项目有这个组件就照抄”。
