...
你仍然在同一个会员券 90 天 GTV 项目里。现在任务推进到「运行环境工程」：业务方不只要答案，还要你说明这个答案为什么可信、什么时候不可信、系统如何避免乱说。

把 Agent 从脚本变成可暂停、可恢复、可审计的系统。
...
DML 工具跑到一半失败时，系统应保留 inspect 和 router 结果，而不是从头丢失。
...
3. 画出 Marketing Agent runtime 草图，并标出状态保存在哪里。
...
运行环境工程 的工业案例只采用来源注册表里已经绑定的资料。你要学的是它们为什么这么设计：复杂系统通过状态、工具、权限、评测或沙箱把模型能力变成可靠工作流；不能学的是“看到某项目有这个组件就照抄”。
