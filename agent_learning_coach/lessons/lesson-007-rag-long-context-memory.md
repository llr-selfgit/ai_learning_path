...
你仍然在同一个会员券 90 天 GTV 项目里。现在任务推进到「检索、长上下文与记忆」：业务方不只要答案，还要你说明这个答案为什么可信、什么时候不可信、系统如何避免乱说。

判断信息应该直接放上下文、检索、长期记忆，还是人工确认。
...
指标口径适合检索，当前字段诊断适合短期状态，你的学习薄弱点适合记忆，用户临时催促不应成为长期规则。
...
3. 为会员券 Agent 写一张知识路由表，列出活动规则、指标口径、历史 bad case、用户偏好分别去哪。
...
检索、长上下文与记忆 的工业案例只采用来源注册表里已经绑定的资料。你要学的是它们为什么这么设计：复杂系统通过状态、工具、权限、评测或沙箱把模型能力变成可靠工作流；不能学的是“看到某项目有这个组件就照抄”。
